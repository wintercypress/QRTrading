...
all_corr <- function(start, end){
        ...
        #sp500_tbl <- sp500 %>% mutate(returns = map(symbol, function(.x) get_return(.x, start, end, 'd')))
...
# start.time <- Sys.time()
# test <- all_corr('2018-01-01', '2018-06-10')
# test2 <- corr_func('MA', test)
# end.time <- Sys.time()
# test2
# 
# t <- datatable(test2, rownames = FALSE,  colnames = capitalize(colnames(test2)), 
#                options = list(dom = 't',
#                columnDefs = list(list(className = 'dt-center', targets = 0:6)))) %>% formatStyle('symbol', fontWeight = 'bold')
# t
